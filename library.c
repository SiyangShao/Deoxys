...
void MixBytes(uint8_t *state) {
    ...
    for (size_t i = 0; i < 4; ++i) {
        for (size_t j = 0; j < 4; ++j) {
            tmp[i + j * 4] = M[0 + i] * state[0 + j * 4] + M[4 + i] * state[1 + j * 4] + M[8 + i] * state[2 + j * 4] + M[12 + i] * state[3 + j * 4];
        }
    }
    for (size_t i = 0; i < 16; ++i) {
        state[i] = tmp[i];
...
void InvMixBytes(uint8_t *state) {
    ...
    for (size_t i = 0; i < 4; ++i) {
        for (size_t j = 0; j < 4; ++j) {
            tmp[i + j * 4] = inv_M[0 + i] * state[0 + j * 4] + inv_M[4 + i] * state[1 + j * 4] + inv_M[8 + i] * state[2 + j * 4] + inv_M[12 + i] * state[3 + j * 4];
        }
    }
    for (size_t i = 0; i < 16; ++i) {
        state[i] = tmp[i];
